...
import type { Metrics, Analysis } from "@/lib/cockpit";
...
type Mem = { screens: any[]; behaviors: any[]; skills: any[]; learnedAt?: string };
...
// The cockpit's live "Overview" — KPIs, the watch-it-think control room, and the
// expandable analyses list. Unchanged behaviour, lifted out of the page so it can sit
// beside the Tree tab.
export function OverviewTab({ m, mem, analyses }: { m: Metrics; mem: Mem; analyses: Analysis[] }) {
  ...
  const kpiCard = (
    ...
    >
      <div style={{ fontFamily: "var(--mono)", fontSize: 10, letterSpacing: ".1em", color: "var(--faint)" }}>{label}</div>
      <div
        style={{
      ...
      <div style={{ fontSize: 12.5, color: opts?.subColor ?? "var(--muted-3)", marginTop: 10 }}>{sub}</div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--muted-2)", fontSize: 15, lineHeight: 1.55, margin: "9px 0 0" }}>
        How the system is learning <b style={{ color: "var(--ink-2)" }}>slab10000/test-app</b> and getting faster — live.
      </p>
        ...
        <div>
          {analyses.map((a, i) => (
            <details className="acc" key={a.key} open={i === 0}>
              <summary>
                <Badge verdict={a.verdict} style={{ flex: "0 0 auto" }}>
                  {a.verdict}
                </Badge>
              ...
              <div className="acc-body">
                {a.description && <p style={{ color: "var(--ink-2)", fontSize: 13.5, lineHeight: 1.6, margin: "12px 0 0" }}>{a.description}</p>}
                  ...
                  <div className="acc-sec">
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 12.5, color: "var(--muted-2)", display: "flex", flexDirection: "column", gap: 4 }}>
                      {a.changedFiles.map((f) => (
                        <div key={f}>
                          <span style={{ color: "var(--accent-2)" }}>M</span>&nbsp; {f}
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {a.codeReview && (
                  <div className="acc-sec">
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 11.5, color: "var(--muted-2)" }}>
                      scope: {a.codeReview.scopeMatch} · risk: {a.codeReview.risk}
                    </div>
                    {a.codeReview.summary && <div style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.55, marginTop: 8 }}>{a.codeReview.summary}</div>}
                    {a.codeReview.concerns.length > 0 && (
                      <ul className="list out" style={{ marginTop: 4 }}>
                        {a.codeReview.concerns.map((x, j) => (
                          <li key={j}>{x}</li>
                        ))}
                      </ul>
                    )}
                  ...
                  <div className="acc-sec" style={{ background: "rgba(52,211,153,.04)", border: "1px solid rgba(52,211,153,.2)", borderRadius: 12, padding: 14 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 11, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontSize: 12, color: "var(--muted-2)" }}>
                        {a.onMerge.merged ? `folded into the baseline ${a.onMerge.mergedAt ?? ""}` : `${a.branch || "this branch"}'s accepted changes fold into main`}
                      </span>
                    </div>
                    ...
                    {!a.onMerge.merged && (
                      <RunTrigger cmd="merge" className="btn btn-ghost" style={{ marginTop: 13 }}>
                        ✓ Approve &amp; merge → main
                      </RunTrigger>
                    )}
                  </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 18, paddingTop: 14, borderTop: "1px solid var(--line-soft)" }}>
                  <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--faint)" }}>
                    {a.tookMs != null ? `analysed in ${fmtTime(a.tookMs)}` : a.generatedAt ? `reviewed ${a.generatedAt}` : "reviewed via GitHub"}
                  </span>
                  <span style={{ flex: 1 }} />
                  {a.href && (
                    <Link href={a.href} className="btn btn-ghost">
                      open trace →
                    </Link>
                  )}
                  {a.githubUrl && (
                    <a href={a.githubUrl} target="_blank" rel="noreferrer" className="btn btn-ghost">
                      view on GitHub →
                    </a>
                  )}
                </div>
      ...
      {/* BOTTOM GRID */}
      <div style={{ display: "grid", gridTemplateColumns: "1.35fr 1fr", gap: 16, marginTop: 24, alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* EXPLORE ONCE, REPLAY FROM CACHE */}
          <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 15, padding: 19 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, letterSpacing: ".12em", color: "var(--faint)" }}>EXPLORE ONCE, REPLAY FROM CACHE</span>
              <span style={{ fontFamily: "var(--display)", fontWeight: 700, fontSize: 15, color: "var(--green)" }}>{cachedMs}ms</span>
            </div>
            <div style={{ marginTop: 16 }}>
              <Sparkline series={[7300, cachedMs]} color="#34d399" invert />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--faint)" }}>~7.3s · first pass (explored)</span>
              <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--faint)" }}>{cachedMs}ms · cached replay</span>
            </div>
            <p style={{ fontSize: 13, color: "var(--muted-2)", lineHeight: 1.5, margin: "13px 0 0" }}>
              The first time the agent reaches a screen it explores with Computer Use and records the route; later passes verify the element
              signature and replay it with zero model calls.
            </p>
          </div>

          {/* PRODUCT MEMORY */}
          <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 15, padding: 19 }}>
            <span style={{ fontFamily: "var(--mono)", fontSize: 11, letterSpacing: ".12em", color: "var(--faint)" }}>PRODUCT MEMORY</span>
            <div style={{ fontSize: 13.5, color: "var(--ink-2)", lineHeight: 1.5, marginTop: 10 }}>
              {m.screens} screens · {m.contracts} contracts · {m.skills} skills · {m.routes} cached routes
            </div>
            <div style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--faint)", marginTop: 8 }}>learned {mem.learnedAt}</div>
          </div>
        </div>

        {/* GRADUATED TO MAIN */}
        <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 15, padding: 19 }}>
          <span style={{ fontFamily: "var(--mono)", fontSize: 11, letterSpacing: ".12em", color: "var(--faint)" }}>GRADUATED TO MAIN</span>
          ...
          <div className="empty" style={{ marginTop: 14 }}>
            Nothing merged yet — accepted skills &amp; contracts fold into the baseline when you merge a PR.
          </div>
        </div>
      </div>
    </div>
